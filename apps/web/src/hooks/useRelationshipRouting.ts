...
const DEFAULT_RELATIONSHIP_STYLE: RelationshipStyle = {
  ...
  labelFontSize: 12,
  labelPadding: 6,
  labelBackgroundColor: 'rgba(248,250,252,0.94)',
  labelTextColor: '#243047',
};
...
export function useRelationshipRouting() {
  ...
  const connectionOffsetsRef = useRef<
  ...
  const getColumnY = useCallback(
    (
      ...
      const columnIndex =
        table.columns?.findIndex(col => col.name === columnName) ?? -1;
      if (columnIndex === -1) {
  ...
  const findTableByName = useCallback(
    (tables: SchemaTable[], targetName: string): SchemaTable | undefined => {
      return tables.find(t => {
        const tableSchema =
          t.schema || (t.name.includes('.') ? t.name.split('.')[0] : undefined);
  ...
  const calculateConnectionOffsets = useCallback(
    (
      ...
      const connectionGroups = new Map<
        string,
        { rel: SchemaRelationship; index: number }[]
      >();
      ...
      connectionGroups.forEach((group, key) => {
        if (group.length > 1) {
          ...
          group.forEach((item, i) => {
            ...
            const existing = offsets.get(item.index) || {
              fromOffset: 0,
              toOffset: 0,
            };
  ...
  const calculateRelationshipPath = useCallback(
    (
      ...
      const isSelected =
        options?.selectedId === relId || options?.highlightedId === relId;
      const offsets = options?.offsets || { fromOffset: 0, toOffset: 0 };

      // Fallback for missing bounds
      if (!fromTableBounds || !toTableBounds) {
        const startX = fromTableBounds
          ? fromTableBounds.x + fromTableBounds.width
          : 150;
        const startY = fromTableBounds
          ? getColumnY(fromTable, rel.fromColumn, fromTableBounds)
          : 100;
        const endX = toTableBounds ? toTableBounds.x : 300;
        const endY = toTableBounds
          ? getColumnY(toTable, rel.toColumn, toTableBounds)
          : 100;
      ...
      const fromColumnY = getColumnY(
        fromTable,
        rel.fromColumn,
        fromTableBounds,
        offsets.fromOffset
      );
      const toColumnY = getColumnY(
        toTable,
        rel.toColumn,
        toTableBounds,
        offsets.toOffset
      ...
      const horizontalOverlap = !(
        fromRight + GAP < toLeft || toRight + GAP < fromLeft
      );
      ...
      if (!horizontalOverlap) {
        ...
        const goRight =
          fromCenterX < toCenterX || fromRight - toLeft < toRight - fromLeft;
  ...
  const calculateAllRelationships = useCallback(
    (
      ...
      return schemaRelationships.map((rel, index) => {
        ...
        return calculateRelationshipPath(
          rel,
          index,
          tableRenderData,
          schemaTables,
