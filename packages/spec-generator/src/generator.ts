...
/**
 * 테이블별 파티션 정보 (DB에서 가져온 데이터)
 */
export interface TablePartitionData {
  schemaName: string;
  tableName: string;
  partitions: PartitionSpecification[];
}
...
export interface GeneratorOptions {
  ...
  /** DB에서 가져온 파티션 데이터 */
  partitionData?: TablePartitionData[];
}

const DEFAULT_OPTIONS: Omit<Required<GeneratorOptions>, 'partitionData'> & { partitionData?: TablePartitionData[] } = {
...
export function generateTableSpecification(
  ...
  // 3.5. 파티션 정보 (외부에서 전달받은 데이터 사용)
  const partitionData = opts.partitionData?.find((p) => {
    const tableFullName = table.schema ? `${table.schema}.${table.name}` : table.name;
    const partitionFullName = p.schemaName ? `${p.schemaName}.${p.tableName}` : p.tableName;
    return tableFullName === partitionFullName || table.name === p.tableName;
  });
